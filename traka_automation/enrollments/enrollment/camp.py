...
class Camp(BaseModel):
    ...
    def cancellation_term_one(self):
        return CancellationTerm(date=self.start_date - CANCELLATION_INTERVAL_ONE, retainer=self.price * 0.25)

    @property
    def cancellation_term_two(self):
        return CancellationTerm(date=self.start_date - CANCELLATION_INTERVAL_TWO, retainer=self.price * 0.5)