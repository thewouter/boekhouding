...
def convert_xlsx_to_pdf(file_path, output_dir):
    ...
    return os.path.join(
        output_dir, os.path.splitext(os.path.basename(file_path))[0] + ".pdf"
    )
    ...
    for directory in main_dir[1]:
        ...
        with open(f"{decla_dir}/data.json") as json_file:
            ...
            invoice_files = sorted(
                [f for f in os.listdir(decla_dir) if f.startswith("bon_")]
            )
            ...
            rendered_latex = latex_template.replace(
                "{{ name }}", tex_escape(data["name"])
            )
            rendered_latex = rendered_latex.replace(
                "{{ cost }}", tex_escape(data["cost"])
            )
            rendered_latex = rendered_latex.replace(
                "{{ IBAN }}", tex_escape(data["IBAN:"])
            )
            rendered_latex = rendered_latex.replace(
                "{{ specification }}", tex_escape(data["specification"])
            )
            rendered_latex = rendered_latex.replace(
                "{{ description }}", tex_escape(data["description"])
            )
            rendered_latex = rendered_latex.replace(
                "{{ comments }}", tex_escape(data["comments"])
            )
            rendered_latex = rendered_latex.replace(
                "{{ submission_time }}", tex_escape(data["submission_time"])
            )
            rendered_latex = rendered_latex.replace(
                "{{ camp }}", tex_escape(data["camp"])
            )
            ...
            shutil.copyfile(
                f"{decla_dir}/{filename}.pdf",
                f"/onedrive/data/exchange_folder/declaratieformulieren/{filename}.pdf",
            )
